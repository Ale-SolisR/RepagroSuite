...
export default function SignaturePad({
  ...
  // Alto generoso del lienzo, responsivo: más grande en pantallas mayores.
  const surfaceCls = 'h-52 w-full sm:h-60'
  ...
  return (
    <div className="rounded-[10px] border border-line bg-paper p-3">
      ...
      {confirmed && value ? (
        <img src={value} alt={`${label} confirmada`} className={`${surfaceCls} rounded-lg border border-line bg-white object-contain`} />
      ) : (
        <canvas
          ref={canvasRef}
          onPointerDown={start}
          ...
          onPointerUp={end}
          onPointerLeave={end}
          className={`${surfaceCls} touch-none rounded-lg border border-dashed border-line bg-white`}
      ...
      <div className="mt-2 flex items-center gap-2">
        <button
          ...
          className="inline-flex min-h-[44px] flex-1 items-center justify-center gap-1.5 rounded-[8px] border border-line px-3 text-[13px] font-medium text-ink transition-colors touch-manipulation hover:bg-bg disabled:opacity-40"
        >
        ...
        {!confirmed && (
          <button
            ...
            className="inline-flex min-h-[44px] flex-1 items-center justify-center gap-1.5 rounded-[8px] px-3 text-[13px] font-medium text-white transition-colors touch-manipulation hover:opacity-90 disabled:opacity-40"
